...
def find_work_time(response_with_code, workday):
    ...
    log.d("find_work_time html", response_with_code.content)
    # resp_html = etree.HTML(content)
    resp_html = etree.HTML(response_with_code.content)

    if "Redmine 500 error" in response_with_code.content.decode("utf-8"):
        clock_in[workday] = "-"
        clock_go_home[workday] = "-"
        delay_map[workday] = 0
        vacations_map[workday] = "请求信息失败，服务器异常"
        return "0.0"
    # print(resp_html)
...
def get_time_at_company(input_month):
    ...
    if response.status_code != 200 and response.status_code != 201:
        log.info_out(f"服务器{response.status_code}异常, 请重试")
        return None
...
def get_work_time(url):
    ...
    if response.status_code != 200 and response.status_code != 201:
        log.info_out(f"服务器{response.status_code}异常, 请重新启动")
        raise Exception(f"服务器{response.status_code}异常, 请重新启动")
    soup = BeautifulSoup(response.content, 'html.parser')
        ...
        if len(item) != 12:
            ...
            if len(item) == 9 and item[4] == '' and item[7] == '':
                ...
                new_item_string = last_item[0] + "\t" + current_holiday_type + "\t" + current_holiday_time \
                                  ...
                                  + "\t" + last_item[5] \
                                  + "\t" + str("%.2f" % (0 if 8 - float(current_day_total) < 0
...
def log_work_time(local_browser, chooses):
    ...
    while len(chooses) != 0:
        ...
        issue_id = input("请输入要登记工时的任务id(退出按q)：")
...
def show_work_report(work_list, worktime_by_days_dict, work_at_weekend):
    ...
    remove_keys = []
    for k, v in clock_in.items():
        if k not in keys:
            remove_keys.append(k)
            clock_go_home.pop(k)
            delay_map.pop(k)
            sorted_vacations_map.pop(k)
    ...
    for key, value in clock_go_home.items():
        ...
        if not leave_day == key:
            if value_to_time > go_home_4_time:
                vacations_map[key] = f"{vacations_map[key]}\n打车可报销\n明天可串休8h"
...
if __name__ == '__main__':
    ...
    try:
        ...
        if work_time_by_days_dict is None:
            raise Exception("统计在岗时间失败")
